...
# conditions are int()
stdp_eq = (
    "clip(weight_e+int((post_time-pre_time)>shift)"
...
# '''
#                     post_time = t
#                     weight_e = clip(weight_e+int((post_time-pre_time)>shift)*p_allowed*amp_potent*exp(((pre_time-post_time)+shift)/tau_pre)+int((post_time-pre_time)<=shift)*p_allowed*amp_depress*exp(clip((post_time-pre_time)-shift, min_delt, clv)/tau_post), min_w, max_w)
# '''
syn_on_post = {
    "plast_on": f"""
                    post_time = t
...
class Config:
    def __init__(self, args):
        ...
        self.path_to_network = args.path_to_network
        self.precomputed_mappings = True
